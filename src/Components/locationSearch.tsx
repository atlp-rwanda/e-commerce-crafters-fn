...
// console.log(location?.longitude, location?.latitude)
  ...
  const handleChangeLocation = async (event: ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      const endpoint = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${event.target.value}&key=AIzaSyAtloVBV7BYHFxASugRGpRmBzmrGnWwnc0`;
  
      const response = await fetch(endpoint);
      const results = await response.json();
      console.log(results)
      setLocSuggestions(results.predictions);
    } catch (error: any) {
            ...
            <div className="absolute bg-gray-100 w-full max-h-36 shadow-sm overflow-auto custom-scrollbar">
              {locSuggestions.map((suggestion, index) => (
                <div
                  ...
                  onClick={() =>
                    handleSelectedLocation(suggestion.description)
                  }
                >
