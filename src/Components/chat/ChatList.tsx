...
const ChatList: React.FC<ChatListProps> = ({
  ...
  useEffect(() => {
    ...
    channel.bind("send-user", (data: any) => {
      const userExists =
        vendors.some((vendor) => vendor.userId === data.sender.userId) ||
        selectUsers.some((user) => user?.userId === data.sender.userId);
      ...
      setLastMessages((prevMessages) => {
        const messageIndex = prevMessages.findIndex(
          (msg) =>
            msg.userId === data.sender.userId ||
            msg.userId === data.message.receiver
        );
        if (messageIndex !== -1) {
        ...
        return [
          ...prevMessages,
          { userId: data.sender.userId, latestMessage: data.message },
        ];
      });
    });
    return () => {
  ...
  const getLatestMessage = (userId: string) => {
    const message = lastMessages.find((msg) => {
      return (
        (msg.latestMessage?.sender === userData.userId &&
          msg.latestMessage?.receiver === userId) ||
        (msg.latestMessage?.sender === userId &&
          msg.latestMessage?.receiver === userData.userId)
      );
    });
    ...
    <div className="w-full bg-white min-h-[45vh] p-2 rounded-[12px] rounded-tl-none flex flex-col gap-[10px]">
      {isLoading ? (
        ...
        <div className=" w-full h-full flex flex-col items-center justify-center">
          ...
          <span className="text-center text-[14px] font-outfit">
            Click plus on top to select vendor
          </span>
        </div>
      ) : (
        <>
          {selectUsers.map((vendor: any) => {
            return (
              <div
                ...
                className=" cursor-pointer hover:opacity-40 flex flex-row gap-[10px]  py-2 items-center"
              >
                ...
                <div className="flex flex-col leading-1 w-[250px]">
                  ...
                  <div className="flex flex-row w-full items-center justify-between gap-[4px]">
                    ...
                    {getUnreadCount(vendor.userId) === 0 ? (
                      ...
                      <span className="bg-primary px-2 text-[12px] text-white rounded-full">
                        {getUnreadCount(vendor.userId)}
                      </span>
                    )}
                  </div>
                </div>
