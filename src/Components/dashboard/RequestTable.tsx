...
import { approveVendor, rejectVendor } from "../../Redux/Admin/requestsActions";
...
const RequestsTable: React.FC<SellerTableProps> = ({ sellers }) => {
  ...
  const onApprove = () => {
    if (selectedSeller) {
      dispatch(approveVendor(selectedSeller.userId));
      closeModal();
    }
  };
  ...
  const onDeny = () => {
    if (selectedSeller) {
      dispatch(rejectVendor(selectedSeller.userId));
      closeModal();
    }
  };
