...
const OrderSummary:React.FC<OrderSummaryProps> = ( {subTotal, deliveryFeePercentage, discountPercentage, total, cartItems, handleDelete}) => {
    return (
        <div className="orderSummary w-[500px] rounded-md hover:shadow-md hover:shadow-black">
            <header className="bg-blue-900 text-white text-center p-3 rounded-t-[5px]">ORDER SUMMARY</header>
            <div className="summaryContent p-5 font-semibold">
            <div className="summaryItems mb-10">
                {cartItems &&
                cartItems.map((item) => (
                    <div className="container  m-[10px] flex justify-between items-center p-[5px] px-[15px] h-[57px] bg-white rounded-md text-[9px] font-semibold mx-auto" key={item.id}>
                        ...
                        <div className="action h-full flex flex-col justify-between items-center">
                            <button className="w-20 h-[25px] bg-blue-900 text-white rounded-[5px] text-[10px]" onClick={() => handleDelete(item.id)}>Remove</button>
                            <div className="flex w-full my-1">
                                <div className="w-[47%] text-amber-600 text-[9px]">Subtotal: </div>
                                <div className="text-left text-[9px] tracking-tight">{item.quantity * item.price} Rwf</div>
                            </div>
                        </div>
                    </div>
                ))}
